...
(() => {
  ...
  document.addEventListener('DOMContentLoaded', ()=>{
    ...
    if (spotifyLoginBtn) {
      ...
      spotifyLoginBtn.innerHTML = '<span style="display:flex;align-items:center;gap:6px"><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M12 3v10.55c-.59-.34-1.27-.55-2-.55-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4V7h4V3h-6z"/></svg>音楽</span>';
      ...
      function showMusicDialog() {
        ...
        dialog.innerHTML = `
          ...
          <div style="display: flex; flex-direction: column; gap: 12px;">
            ...
            <button id="spotifyUrlBtn" class="btn" style="width: 100%; background: linear-gradient(135deg, #1DB954 0%, #1ed760 100%);">Spotify URL</button>
            <button id="youtubeUrlBtn" class="btn" style="width: 100%; background: linear-gradient(135deg, #FF0000 0%, #CC0000 100%);">YouTube URL</button>
        ...
        document.getElementById('youtubeUrlBtn').addEventListener('click', () => {
          ...
          const url = prompt(
            ...
            'または: https://music.youtube.com/watch?v=...'
          );
      ...
      function showMusicSearch() {
        ...
        document.getElementById('searchYouTubeBtn').addEventListener('click', async () => {
          ...
          try {
            // YouTube Data API v3を使用（簡易版：直接検索URLを開く）
            ...
            resultsDiv.innerHTML = `
              ...
              <p style="color: var(--muted); font-size: 0.85rem; margin-top: 12px;">
                ※動画を見つけたら、URLをコピーして「YouTube URL」から再生してください
              </p>
            `;
          } catch (error) {
            resultsDiv.innerHTML = '<p style="color: #ff4444;">検索に失敗しました</p>';
          }
        });
